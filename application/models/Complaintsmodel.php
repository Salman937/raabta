...
class Complaintsmodel extends CI_Model
{
    ...
    function my_complaints($table, $id, $search = "by_user_id")
    {
        $this->db
            ->select("*")
            ->from($table)
            ->join('complaint_types', 'complaint_types.complaint_type_id = complaints.complaint_type_id')
            ->join('complaints_status', 'complaints_status.complaints_status_id = complaints.complaints_status_id')
            ->join('complaint_response', 'complaint_response.complaint_id = complaints.complaint_id')
            ->join('signup', 'signup.signup_id = complaints.signup_id');
        if ($search == "by_complaint_id") {
            $this->db->where('complaints.complaint_id', $id);
        } else {
            $this->db->where('user_id', $id);
        }
        $this->db->order_by('complaints.complaint_id', 'DESC');
